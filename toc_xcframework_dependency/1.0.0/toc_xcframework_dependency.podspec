Pod::Spec.new do |s|
	s.platform              = :ios
	s.ios.deployment_target = "13.0"
	s.swift_version         = "5"
	s.requires_arc          = true
	s.name                  = "toc_xcframework_dependency"
	s.version               = "1.0.0"
	s.homepage              = "https://github.com/diegovilloutafredes/toc_xcframework_dependency"
	s.author                = { "Diego Villouta" => "diegov17@gmail.com" }
	s.summary               = "Test library Test library Test library Test library Test library Test library Test library Test library Test library"
	s.license               = { :type => "MIT", :file => "LICENSE" }
	s.source                = { :git => "https://github.com/diegovilloutafredes/toc_xcframework_dependency.git", :tag => "#{s.version}" }
	s.frameworks            = "UIKit"
	s.vendored_frameworks   = "SocketIO.xcframework"
end